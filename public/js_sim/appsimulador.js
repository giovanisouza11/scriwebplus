...
function Sim_Draw_Inicio() {
	...
	Sim_Canvas.width = 570; 
	Sim_Canvas.height = 528; 
}
  	...
  	for(var funcao = 0; funcao < 6; funcao++)
	{
	    switch (parseInt(funcao)){
      		case 0:
        		...
        		Ihm_Context.fillText('ENTRADAS', 280, linha);
        		tamanho_array = I.length;
        		num_linhas = parseInt(tamanho_array / 16);
        		linha = linha + 15;
        		Ihm_Context.fillText('MSB', 280,linha);
        		Ihm_Context.fillText('Decimal', 530,linha);
			linha = linha + 15;
        		break;
      		case 1:
        		Ihm_Context.fillStyle = 'black';
        		Ihm_Context.fillRect(280, linha-5, 60, 5);
        		Ihm_Context.fillText('SAIDAS', 280, linha);
      		 ...
      		 case 2:
        		Ihm_Context.fillStyle = 'black';
        		Ihm_Context.fillRect(790, linha-5, 60, 5);
        		Ihm_Context.fillText('MEMÓRIAS', 870, linha);
        		Ihm_Context.fillRect(955, linha-5, 60, 5);
        		Ihm_Context.fillRect(1025, linha-5, 60, 5);
        		tamanho_array = M.length;
        		num_linhas = parseInt(tamanho_array / 16);
        		linha = linha + 15;
        		Ihm_Context.fillText('MSB', 790,linha);
        		Ihm_Context.fillText('Valor binário', 870,linha);
        		Ihm_Context.fillText('LSB', 990,linha);
        		Ihm_Context.fillText('Decimal', 1035,linha);
        		linha = linha + 15;
        		break;
		case 3:
        		Ihm_Context.fillStyle = 'black';
        		Ihm_Context.fillText('TIMERS', 20, linha);
       			tamanho_array = T.length;
        		num_linhas = parseInt(tamanho_array / 9);
        		Ihm_Context.fillText('Tem', 70,linha);
        		Ihm_Context.fillText('bit', 100,linha);
        		Ihm_Context.fillText('PV', 130,linha);
        		Ihm_Context.fillText('SP', 160,linha);
        		break;
		case 4:
        		Ihm_Context.fillStyle = 'black';
        		Ihm_Context.fillRect(790, linha-5, 60, 5);
        		Ihm_Context.fillText('CONTADORES', 860, linha);
        		Ihm_Context.fillRect(955, linha-5, 60, 5);
        		tamanho_array = C.length;
        		num_linhas = parseInt(tamanho_array / 4);
        		linha = linha + 15;
        		Ihm_Context.fillText('Cont', 790,linha);
        		Ihm_Context.fillText('bit', 845,linha);
        		Ihm_Context.fillText('PV', 895,linha);
        		Ihm_Context.fillText('SP', 945,linha);
        		Ihm_Context.fillText('AUX', 990,linha);
        		linha = linha + 15;
        		break;
      		case 5:
        		Ihm_Context.fillStyle = 'black';
        		Ihm_Context.fillRect(790, linha-5, 60, 5);
        		Ihm_Context.fillText('RAMAIS', 880, linha);
        		Ihm_Context.fillRect(955, linha-5, 60, 5);
	   ...
	   for (var n_linhas=0; n_linhas <= num_linhas; n_linhas++) {
      			...
      			if (n_linhas < num_linhas){
        			if (funcao <3){
        				Ihm_Context.fillText(tipo_funcao[funcao]+n_linhas+'.15', 280,linha);
        				n_posicao = (n_linhas * 16) + posicao;
        			}
        			if (funcao==5) {
        				n_posicao = (n_linhas * 16) + posicao;
        				Ihm_Context.fillText(tipo_funcao[funcao]+(n_posicao+15), 280,linha);
        			}
      			}
      			else  {
        			if (funcao <3) {
        				Ihm_Context.fillText(tipo_funcao[funcao]+n_linhas+'.'+((tamanho_array%16)-1), 280,linha);
        				n_posicao = (n_linhas * 16) + posicao;
        			}
        			if (funcao==5) {
        				n_posicao = (n_linhas * 16) + posicao;
        				Ihm_Context.fillText(tipo_funcao[funcao]+(n_posicao + (tamanho_array%16)-1), 280,linha);
        			}
      			}
      			if(funcao==4) {
			...
			//	
      			...
      			do {
        			...
        			case 0:
        				Ihm_Context.fillStyle = 'red';
        				Ihm_Context.fillText(I[n_posicao], 530-(posicao*10),linha);
        				break;
        			case 1:
        				Ihm_Context.fillStyle = 'red';
        				Ihm_Context.fillText(Q[n_posicao], 530-(posicao*10),linha);
        				break;
        			case 2:
        				Ihm_Context.fillStyle = 'red';
        				Ihm_Context.fillText(M[n_posicao], 980-(posicao*10),linha);
        				break;
        			case 5:
        				Ihm_Context.fillStyle = 'red';
        				Ihm_Context.fillText(R[n_posicao], 980-(posicao*10),linha);
        				break;
        			case 4:
        				Ihm_Context.fillStyle = 'red';
        				Ihm_Context.fillText(C[n_posicao], 850+(posicao*50),linha);
        				if (posicao >2)
        					posicao = 15;
        				break;
        			case 3:
      			...
      			if (funcao<3) {
        			Ihm_Context.fillStyle = 'green';
        			Ihm_Context.fillText(tipo_funcao[funcao]+n_linhas+'.0 - ' +tipo_funcao[funcao]+n_linhas+' = ', 530,linha);
        			Ihm_Context.fillStyle = 'red';
        			Ihm_Context.fillText(Sim_Endereco_CT(tipo_funcao[funcao]+n_linhas,0), 540,linha);
      			}
      			if (funcao==5) {
        			Ihm_Context.fillStyle = 'green';
        			Ihm_Context.fillText(tipo_funcao[funcao]+(n_linhas*16), 530,linha);
      			}
      			linha += 15;
...
function Sim_Draw_Fundo() {
	...
	Sim_Context.fillStyle = 'white';
	Sim_Context.fillRect(0, 0, 570, 528);
	Sim_Context.fillStyle = 'black';
	Sim_Context.strokeRect(1, 1, 569, 527);
}
