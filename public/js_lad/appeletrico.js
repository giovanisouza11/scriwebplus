var AImage1 = new Image();
AImage1.src = "/img_lad/eletrico.png";
var valor_botao = new Array(7);
var M = new Array();
var I = new Array();
var R = new Array();
var Q = new Array();
var T = new Array();
var C = new Array();
var localizacao;
var comandos = 0; // 0:stop, 1: run 2: run ciclo, 3:Run Passo
var canvas;
var context;

function draw_eletrico() {
    canvas = document.getElementById("tela1");
	context= canvas.getContext("2d");
    canvas.width = 425;
    canvas.height = 500;
    canvas.top = 200;
	var i;
    for(i=0; i<6; i++){
	   	valor_botao[i] = 0;
		I[i] = 0;
	}
    for(i=0; i<6; i++)
       	context.drawImage(AImage1, (AImage1.width/5)*1, 0, AImage1.width/5, 110, 62+(i*60), 0, AImage1.width/5, 110);
	desenha_clp();
	for(i=0; i<6; i++)
		context.drawImage(AImage1, (AImage1.width/5)*3, 110, AImage1.width/5, 110, 62+i*60, 330, AImage1.width/5, 110);
    for(i=0; i<31; i++)
		valor_chave[i] = 0;
}

function desenha_clp(){
	context.font = '9pt Arial';
	context.lineWidth = 1;
	context.fillStyle = 'ivory';
	context.fillRect(10, 110, 410, 220);
	context.fillStyle = 'black';
	context.strokeRect(10, 110, 410, 220);
	context.lineWidth = 0.3;
	context.strokeRect(10, 130, 410, 40);
	context.strokeRect(10, 270, 410, 40);
	context.lineWidth = 1;
	for(var i=0; i<7; i++)
		desenha_borne_clp(i);

	context.fillStyle = 'red';
	context.fillRect(27, 201, 32, 18);
	context.fillStyle = 'black';
	context.strokeRect(25, 199, 36, 22);
	context.fillText('Power', 25, 193);
	context.strokeRect(25, 239, 36, 22);
	context.fillText('Run', 31, 233);
	context.fillText('Entradas - I0.', 86, 182);
	context.fillText('Memórias - M0.', 76, 199);
	context.fillText('M1.', 139, 209);
	context.fillText('M2.', 139, 219);
	context.fillText('Dervação - TR', 78, 245);
	context.fillText('Saídss  - Q0.', 90, 263);
	liga_led_CLP();


}

function desenha_borne_clp(i){
	context.lineWidth = 1;
	context.fillStyle = 'black';
	context.strokeRect(20+(i*62), 130, 28, 20);
	context.strokeRect(20+(i*62), 290, 28, 20);
	context.lineWidth = 0.3;
	context.strokeRect(20+(i*62), 310, 28, 20);
	context.strokeRect(20+(i*62), 110, 28, 20);
	context.lineWidth = 0.5;
	context.shadowColor = 'black';
	context.shadowOffsetX = 1;
	context.shadowOffsetY = 1;
	context.strokeRect(28+(i*62), 113, 14, 14);
	context.strokeRect(28+(i*62), 313, 14, 14);
	context.beginPath();
	context.ellipse(35+(i*62), 120, 5, 5, 0, 0, 359);
	context.ellipse(35+(i*62), 120, 5, 2, i*60, 0, 359);
	context.stroke();
	context.beginPath();
	context.ellipse(35+(i*62), 320, 5, 5, 0, 0, 359);
	context.ellipse(35+(i*62), 320, 5, 2, i*60, 0, 359);
	context.stroke();
	context.lineWidth = 1;
	context.shadowOffsetX = 0;
	context.shadowOffsetY = 0;

	if (i==0){
		context.fillText('COM', 20, 165);
		context.fillText('COM', 20, 285);
	}
	else {
		context.fillText('I0.'+(i-1), 21+(i*62), 165);
		context.fillText('Q0.'+(i-1), 21+(i*62), 285);
	}
}



function draw_chave(posicaox, posisicaoy) {
        canvas = document.getElementById("tela1");
	context= canvas.getContext("2d");
	var index = posicaox;
	var i=posicaox;
	context.drawImage(AImage1, (AImage1.width/5)*(valor_botao[index]+1), 0, AImage1.width/5, 110, 62+i*60, 0, AImage1.width/5, 110);
}

function trocar() {
    var posicaox;
	posicaox = parseInt((window.event.clientX -62 )/ 60);
    var posicaoy=window.event.clientY;
	var index = posicaox;

    if ((posicaox >=0) && posicaox < 6)
    {
        if (valor_botao[index]==0) {
            valor_botao[index]=1;
		}
		else{
			valor_botao[index]=0;
		}
		draw_chave(posicaox, posicaoy);
	}
	envia_entrada(valor_botao);
	run_CLP();
}

function run_CLP(){
	if(comandos == 1) {
		context.fillStyle = 'green';
		context.fillRect(27, 241, 32, 18);
		context.font = '16pt Arial';
		context.fillStyle = 'black';
		context.fillText('A', 36, 259);
		liga_led_CLP();
	}
	if(comandos == 2) {
		context.fillStyle = 'green';
		context.fillRect(27, 241, 32, 18);
		context.font = '16pt Arial';
		context.fillStyle = 'black';
		context.fillText('1', 36, 259);
		liga_led_CLP();
	}
	if(comandos == 3) {
		context.fillStyle = 'green';
		context.fillRect(27, 241, 32, 18);
		context.font = '16pt Arial';
		context.fillStyle = 'black';
		context.fillText('P', 36, 259);
		liga_led_CLP();
	}
}

function liga_led_CLP(){
	for(var i=0; i <6; i++){
		if (valor_botao[i]==1)
			context.fillStyle = 'green';
		else
			context.fillStyle = 'white';
		context.fillRect(84+(i*62), 132, 24, 16);
	}
	context.font = '9pt Arial';

	for(var i=15; i >=0; i--){
		var j = (i<10) ? 4 : 0;
		if (I[i]==1)
			context.fillStyle = 'green';
		else
			context.fillStyle = 'gray';
		context.fillRect(403-(i*16), 177, 14, 5);
		context.fillText(i, j+403-(i*16), 192);
		if (M[i]==1)
			context.fillStyle = 'green';
		else
			context.fillStyle = 'gray';
		context.fillRect(403-(i*16), 194, 14, 5);
		if (M[i+16]==1)
			context.fillStyle = 'green';
		else
			context.fillStyle = 'gray';
		context.fillRect(403-(i*16), 203, 14, 5);
		if (M[i+32]==1)
			context.fillStyle = 'green';
		else
			context.fillStyle = 'gray';
		context.fillRect(403-(i*16), 213, 14, 5);
		if (R[i]==1)
			context.fillStyle = 'green';
		else
			context.fillStyle = 'gray';
		context.fillRect(403-(i*16), 240, 14, 5);
		if (Q[i]==1)
			context.fillStyle = 'green';
		else
			context.fillStyle = 'gray';
		context.fillRect(403-(i*16), 258, 14, 5);
		context.fillText(i, j+403-(i*16), 256);
	}
	if(comandos != 0) {
		for(var i=0; i <6; i++){
			if (Q[i]==1)
				context.fillStyle = 'green';
			else
				context.fillStyle = 'white';
			context.fillRect(84+(i*62), 292, 24, 16);

			context.drawImage(AImage1, (AImage1.width/5)+(AImage1.width/5)*Q[i], 110, AImage1.width/5, 110, 62+i*60, 330, AImage1.width/5, 110);
		}
	}

}

function stop_CLP(){
	context.fillStyle = 'red';
	context.fillRect(27, 242, 32, 16);
	for(var i=0; i <6; i++){
		context.fillStyle = 'white';
		context.fillRect(84+(i*62), 132, 24, 16);
	}
	for(var i=0; i <6; i++){
		context.fillStyle = 'white';
		context.fillRect(84+(i*62), 292, 24, 16);
		context.drawImage(AImage1, (AImage1.width/5)*0, 110, AImage1.width/5, 110, 62+i*60, 330, AImage1.width/5, 110);
	}
}
